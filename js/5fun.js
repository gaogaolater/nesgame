...
class Speakers {
    ...
    start() {
        ...
        var realbuffersize = SCRIPT_BUFFER_SIZE;
        if (navigator.userAgent.indexOf("iPhone") != -1 || navigator.userAgent.indexOf("iPad") != -1) {
            realbuffersize = 1024
        }
        this.scriptNode = this.audioCtx.createScriptProcessor(realbuffersize, 0, 2);
...
function joystickInit(realWidth, nesWidth, realHight, type) {
    ...
    if ((realHight - nesWidth) / 2 > (minwidth - 10)) {
        ...
        if (joyarea > realWidth / 2 - 5) joyarea = realWidth / 2 - 5;
        joymargin = 10;
        joyopacity = 1
    }
...
function touchInit(type) {
    ...
    $('.interaction-area').bind('touchend', function (e) {
        ...
        nesButtonUp({
            keyCode: jsnes.Controller.BUTTON_LEFT
        }, player);
        nesButtonUp({
            keyCode: jsnes.Controller.BUTTON_UP
        }, player);
        nesButtonUp({
            keyCode: jsnes.Controller.BUTTON_RIGHT
        }, player);
        nesButtonUp({
            keyCode: jsnes.Controller.BUTTON_DOWN
        }, player)
    });
...
function initSavelist(gid) {
    ...
    $('.tenant-model-content').css({
        ...
        "left": -($(window).height() * 0.9 - $(window).width()) / 2 + 'px'
    });
...
function getCheatCode() {
    var id = gameid //getQueryString("gameId"); //  通过URL的gameid参数获取对应的金手指
...
function initCheatCon() {
    ...
    if (gamecheatList.length > 0) {
        for (var i = 0; i < gamecheatList.length; i++) {
            $('#cheatlist').append('<li><span class="name" style="margin-top: 5px">' + gamecheatList[i].desc + '</span><label style="width:100%;"><input type="checkbox" onchange="changecheat(this.checked,' + i + ')" class="ios-switch green  bigswitch" /><div class="cheatswitch" style="float:right;"><div></div></div></label></li>')
        }
    } else {
        $('#cheatlist').append("<div class='notjsz'>暂无金手指</div>");
    }
}
function changecheat(checkstatus, num) {
    gamecheatList[num].status = checkstatus;
    // console.log(checkstatus, num,'选择状态:',gamecheatList[num].lastmem, nes.cpu.mem[gamecheatList[num].address])
...
function savehistory() {
    var canvas = document.getElementById("nes-canvas");
    var nesInfo = new Object();
    nesInfo.name = getQueryString("gameName");
    nesInfo.id = gameid;
